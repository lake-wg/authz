...
The authenticator and authorization server are assumed to have a secure connection, for example TLS 1.3 authenticated with certificates. The authenticator is assumed to authenticate with the public key PK_V, see {{domain-auth}}.
...
The autenticator associates the nonce G_X to the secure connection.
If G_X is not unique among nonces associated to this connection, the protocol SHALL be discontinued.
...
If G_X is not unique amonce nonces associated to this identity, the protocol SHALL be discontinued.
